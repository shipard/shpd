...
| `DataSourceConfig` | Načtení a validace `config/main.json` zdroje dat (ID, DB credentials, moduly, defaultLanguage). Volitelná pole: `defaultLanguage` (default `en`), `defaultCurrency` (default `czk`), `skipProvisioning` (bool, default `false` — dočasně vypne auto-provisioning v `ds-upgrade`; viz `docs/cli.md`), `enableReset` (bool, default `false` — povolí `ds-reset` na produkčním serveru pro testovací DS; viz `docs/cli.md`). |
