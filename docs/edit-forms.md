...
| `DocsHeadsFormBase` (`renderRecapRows`) | `recap` | Převzatá rekapitulace DPH (`vat_recap_source = 1`): # · Kód DPH · DPH % · Základ · Daň · Celkem, u cizí měny navíc trojice v domácí měně (ke čtení — dopočítá je uložení z kurzu). Částky, které do součtů hlavičky nevstupují (`sum_*` z definice kódu — typicky oddaňovací pár samovyměření), jsou tlumené. **Tab má dvě podoby:** u přepočítané rekapitulace zůstává HTML přehled (editovat vypočtené nemá smysl), u převzaté je to tahle sub-tabulka nad `docs_core_vat_recap` s formem `docs.core.vatRecap`. Přepínač `vat_recap_source` v sekci DPH má `triggers: reload`, aby se tab překreslil. Viz `docs/vat-calculation.md` § 5. |
