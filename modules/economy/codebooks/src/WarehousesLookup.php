...
/**
 * Lookup pro sklady (economy_codebooks_warehouses). Hledá v kódu a názvu;
 * display: `kód — název`.
 *
 * Používá ho formulář číselné řady (vazba řady na sklad — mechanismus je
 * připravený, typ dokladu se `series_binding: warehouse` zatím neexistuje).
 */
class WarehousesLookup extends TableLookup
{
    ...
    public function search(string $q, array $filter, int $limit): array
    {
        ...
        $sql = 'SELECT `id`, `code`, `name` FROM `economy_codebooks_warehouses`'
    ...
    public function resolve(array $ids): array
    {
        ...
        $rows = $this->db->fetchAll(
            'SELECT `id`, `code`, `name` FROM `economy_codebooks_warehouses` WHERE `id` IN %in',
            array_values($intIds),
        );
        return array_map(fn($r) => $this->buildItem($r), $rows);
    }
