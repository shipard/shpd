...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        -h|--help)
            cat <<EOF
Usage: sudo bash scripts/install-packages.sh --mode=<development|production>
EOF
            exit 0
            ;;
        *)
...
sed "s|fastcgi_pass unix:/run/php/[^;]*;|fastcgi_pass unix:${POOL_SOCKET};|g" "$TEMPLATE" > "$SITE_FILE"
