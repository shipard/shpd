...
use Shipard\Command\DataSource\HostingAnalyzerKeyCommand;
...
class TestableHostingAnalyzerKeyCommand extends HostingAnalyzerKeyCommand
{
...
class HostingAnalyzerKeyCommandTest extends TestCase
{
    ...
    {
        $this->tempDir = sys_get_temp_dir() . '/shpd_analyzerkey_test_' . uniqid();
    ...
    {
        $command = new TestableHostingAnalyzerKeyCommand(
            $this->dsConfig,
            $this->dsConnection,
            $this->tempDir,
        );
    ...
    public function testGenerateStoresPrefixAndHashAndPrintsTokenOnce(): void
    {
        ...
        $this->dsConnection->expects($this->once())
            ...
            ->willReturnCallback(function (string $table, array $data) use (&$captured): void {
                $this->assertSame('hosting_core_ai_analyzers', $table);
                $captured = $data;
            });

        $tester = $this->tester();
        $exitCode = $tester->execute(['--analyzer' => '3', '--generate' => true]);

        $this->assertSame(Command::SUCCESS, $exitCode);
        $output = $tester->getDisplay();

        $this->assertStringContainsString('API key generated: Analyzer EU-1', $output);
    ...
    public function testRevokeClearsPrefixAndHash(): void
    {
        ...
        $tester = $this->tester();
        $exitCode = $tester->execute(['--analyzer' => '3', '--revoke' => true]);
    ...
    public function testRequiresAnalyzerOption(): void
    {
        $tester = $this->tester();
        $exitCode = $tester->execute(['--generate' => true]);

        $this->assertSame(Command::FAILURE, $exitCode);
        $this->assertStringContainsString('--analyzer <id> is required', $tester->getDisplay());
    }
    ...
    public function testRequiresExactlyOneAction(): void
    {
        ...
        $exitCode = $tester->execute(['--analyzer' => '3']);
        $this->assertSame(Command::FAILURE, $exitCode);
        $this->assertStringContainsString('exactly one of --generate or --revoke', $tester->getDisplay());

        $exitCode = $tester->execute(['--analyzer' => '3', '--generate' => true, '--revoke' => true]);
        $this->assertSame(Command::FAILURE, $exitCode);
    }

    public function testFailsOnUnknownAnalyzer(): void
    {
        $this->dsConnection->method('fetchRow')->willReturn(null);
        $this->dsConnection->expects($this->never())->method('updateWhere');

        $tester = $this->tester();
        $exitCode = $tester->execute(['--analyzer' => '9', '--generate' => true]);

        $this->assertSame(Command::FAILURE, $exitCode);
        $this->assertStringContainsString("analyzer '9' not found", $tester->getDisplay());
    }
