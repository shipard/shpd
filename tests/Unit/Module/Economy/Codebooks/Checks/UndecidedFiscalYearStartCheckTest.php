...
use Shipard\Module\Economy\Codebooks\Checks\UndecidedFiscalYearStartCheck;

class UndecidedFiscalYearStartCheckTest extends TestCase
{
    /** @param array<string, mixed> $settings klíč → hodnota; chybějící klíč = nerozhodnuto */
    private function makeCheck(array $settings): UndecidedFiscalYearStartCheck
    {
        ...
        return new UndecidedFiscalYearStartCheck(
            $db,
    ...
    public function testDecidedIsSilent(): void
    {
        $this->assertSame([], $this->makeCheck(['economy.fiscalYearStartMonth' => 1])->run());
    }
}
