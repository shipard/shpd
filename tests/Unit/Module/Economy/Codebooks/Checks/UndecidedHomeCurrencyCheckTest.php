...
use Shipard\Module\Economy\Codebooks\Checks\UndecidedHomeCurrencyCheck;

class UndecidedHomeCurrencyCheckTest extends TestCase
{
    /** @param array<string, mixed> $settings klíč → hodnota; chybějící klíč = nerozhodnuto */
    private function makeCheck(array $settings): UndecidedHomeCurrencyCheck
    {
        ...
        return new UndecidedHomeCurrencyCheck(
            $db,
    ...
    {
        $this->assertSame([], $this->makeCheck(['economy.homeCurrency' => 'czk'])->run());
    }
}
